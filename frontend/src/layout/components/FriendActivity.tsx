          ...
          {[...users]
            .filter((user) => {
              ...
              const isListening =
                ...
                userActivities.get(user.clerkId) !== "Idle";

              if (showOnlyOnline && !isOnline) return false;
              if (showOnlyListening && !isListening) return false;
            ...
            .sort((a, b) => {
              ...
              return Number(bOnline) - Number(aOnline); // online đứng trước
            })
            .map((user) => {
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        ...
                        {isPlaying && (
                          <Music className="size-3.5 text-yellow-400 shrink-0" />
                        )}
                      </div>
...
const LoginPrompt = () => (
  <div className="h-full flex flex-col items-center justify-center p-6 text-center space-y-4">
    <div className="relative">
      <div
        className="absolute -inset-1 bg-gradient-to-r from-yellow-500 to-red-400 rounded-full blur-lg
       opacity-75 animate-pulse"
        aria-hidden="true"
      />
      <div className="relative bg-zinc-900 rounded-full p-4">
        <HeadphonesIcon className="size-8 text-yellow-500" />
      </div>
    </div>
